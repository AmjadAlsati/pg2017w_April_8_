...
class RCGamepadFrame(RCBaseIO):
    def __init__(self, master=None, init_data=None, widget_in_tab= None, widget_out_tab=None,
        ...
        self.get_init_tab = {'get_x': (self.get_x, None),
                             ...
                             'get_gamepad_pos': (self.get_gamepad_pos, None)}
    ...
    def create_widgets(self):         
        ...
        ### dummy
        self.popup.add_command(label='Init Gamepad', command = self.cmd_init_gamepad)        

    def get_installed_widgets(self):
        widget_structure = ['Gamepad', {'widget_in_tab':self.widget_in_tab,
    ...
    def get_gamepad_pos(self):
        """
        Get axis values from gamepad
        """
        pygame.event.pump()
        self.x_in = self.gamepad.get_axis(0)
        self.y_in = self.gamepad.get_axis(1)
        self.set_act_in_pos()
        ret = (self.x_in, self.y_in)
        return ret

    def cmd_init_gamepad(self):
        """
        ...
        self.gamepad.init()
        ax_num = self.gamepad.get_numaxes()
#        but_num = self.gamepad.get_numbuttons()

        if ax_num < 2:
            print("ERROR: The gamepad has not enough axis!")

    def set_x(self, x_out):
        self.x_out = float(x_out)
        if self.x_out > 1.0: self.x_out = 1.0
        self.set_act_out_pos()

    def set_y(self, y_out):
        self.y_out = float(y_out)
        if self.y_out > 1.0: self.y_out = 1.0
        self.set_act_out_pos()
